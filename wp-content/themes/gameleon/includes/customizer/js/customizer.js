jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					tiguanAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			tiguanGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				tiguanGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			tiguanGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	function tiguanAppendRow($element, defaultValue = '') {
	...
	function tiguanGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  tiguanGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function tiguanGetAllImageCheckboxes($element) {
	...
	$( ".liviu_checkbox_control .sortable" ).sortable({
		placeholder: "liviu-ui-state-highlight",
		update: function( event, ui ) {
			tiguanGetAllLiviuCheckboxes($(this).parent());
		}
	});

	$('.liviu_checkbox_control .sortable-liviu-checkbox').on('change', function () {
		tiguanGetAllLiviuCheckboxes($(this).parent().parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function tiguanGetAllLiviuCheckboxes($element) {
		var inputValues = $element.find('.sortable-liviu-checkbox').map(function() {
			if( $(this).is(':checked') ) {
				return $(this).val();
			}
		}).toArray();
		$element.find('.customize-control-sortable-liviu-checkbox').val(inputValues).trigger('change');
	}
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.tiguanfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.tiguanfontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.tiguanfontslist[index].category);

		tiguanGetAllSelects($(this).parent().parent());
	});
	...
	function tiguanGetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].tiguantinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].tiguantinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].tiguanmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['tiguan-upsell'] = api.Section.extend( {
