...
function arqam_add_admin() {
	...
	if( isset( $_REQUEST['action'] ) && $current_page == 'arqam' ){
		...
		elseif( 'foursquare' == $_REQUEST['action'] ){
		...
		elseif( 'linkedin' == $_REQUEST['action']  ){
		...
		elseif( 'dribbble' == $_REQUEST['action']  ){
			...
			$cur_page =  urlencode ( admin_url().'admin.php?page=arqam&service=arq-dribbble' );
...
if( isset( $_REQUEST['service'] ) && 'arq-twitter' == $_REQUEST['service'] && $current_page == 'arqam' ){
?>
...
elseif( isset( $_REQUEST['service'] ) && 'arq-dribbble' == $_REQUEST['service'] && $current_page == 'arqam' ){

		if( !empty( $_REQUEST['code'] ) ){
		$code                    = $_REQUEST['code'];
		$cur_page                = admin_url().'admin.php?page=arqam&service=arq-dribbble' ;
	...
	<form method="post">
		<div id="poststuff">
			<div id="post-body" class="metabox-holder columns-2">
				<div id="post-body-content" class="arq-content">
					<div class="postbox">
						<h3 class="hndle"><span><?php _e( 'Dribbble App info' , 'arq' ) ?></span></h3>
						<div class="inside">
							<table class="links-table" cellpadding="0">
								<tbody>
									<tr>
										<th scope="row"><label for="client_id"><?php _e( 'Consumer key' , 'arq' ) ?></label></th>
										<td><input type="text" name="client_id" id="client_id" value=""></td>
									</tr>
									<tr>
										<th scope="row"><label for="client_secret"><?php _e( 'Consumer secret' , 'arq' ) ?></label></th>
...
elseif( isset( $_REQUEST['service'] ) && 'arq-twitch' == $_REQUEST['service'] && $current_page == 'arqam' ){
	...
	if( !empty( $_REQUEST['code'] ) ){
		...
		$client_id     = get_transient( 'arq_twitch_client_id' );
		$client_secret = get_transient( 'arq_twitch_client_secret' );
		...
		if( ! empty( $response->access_token ) ){
			...
			update_option( 'twitch_client_id',     $client_id );
			update_option( 'twitch_client_secret', $client_secret );
		}
...
elseif( !empty( $_REQUEST['debug'] ) && $current_page == 'arqam' ){
	...
	if ( ! get_option( 'tie_token_'.ARQAM_PLUGIN_ID ) ) {
		return;
	}
						...
						<div class="inside">
							<table class="links-table" cellpadding="0">
								<tbody>
									...
									<tr>
										<th scope="row"><label for="social[facebook][number]"><?php esc_html_e( 'Number of Fans', 'arq' ) ?></label></th>
							...
							<div>
								<strong><?php _e( 'Need Help?' , 'arq' ) ?></strong><p><em><?php _e( 'Enter Your Facebook Page Name', 'arq' ); ?> </em></p>
							</div>
				<?php
					break;
					case 'twitter':
						...
						<div class="inside">
							<table class="links-table" cellpadding="0">
								<tbody>
									...
									<tr>
										<th scope="row"><label for="social[twitter][number]"><?php esc_html_e( 'Number of Followers', 'arq' ) ?></label></th>
								...
								<tbody>
									...
									<tr>
										...
										<td>
											...
											<a class="button-large button-primary tie-get-api-key" href="<?php echo admin_url().'admin.php?page=arqam&service=arq-dribbble' ?>"><?php _e( 'Get Access Token' , 'arq' ) ?></a>
										</td>
						...
						<div class="inside">
							<table class="links-table" cellpadding="0">
								<tbody>
									...
									<tr>
										<th scope="row"><label for="social[instagram][number]"><?php esc_html_e( 'Number of Followers', 'arq' ) ?></label></th>
										<td><input type="text" name="social[instagram][number]" class="code" id="social[instagram][number]" value="<?php if( !empty($arq_options['social']['instagram']['number']) ) echo $arq_options['social']['instagram']['number'] ?>"></td>
									</tr>
									<tr>
										<th scope="row"><label for="color[<?php echo $arq_item; ?>]"><?php _e( 'Custom color' , 'arq' ) ?> </label></th>
										<td><input type="text" name="color[<?php echo $arq_item; ?>]" value="<?php if( !empty( $arq_options['color'][$arq_item] ) ) echo ( $arq_options['color'][$arq_item] ); ?>" class="arq-color-field" ></td>
							...
							<div>
								<strong><?php _e( 'Need Help?' , 'arq' ) ?></strong><p><em><?php _e( 'Enter Your Instagram Username and the number of followers.', 'arq' ) ?></em></p>
								<p><em><a href="https://developers.facebook.com/docs/instagram-basic-display-api/reference/user#edges" target="_blank"><?php _e( 'Instagram has removed the number of followers from their API.', 'arq' ) ?></a></em></p>
							</div>
				<?php
					break;
					case 'mailchimp':
...
function tie_arqam_add_admin_init(){
	...
	wp_localize_script( 'jquery-ui-core', 'tiearqam_js', $arqam_translation_array );
}

add_action('admin_init', 'tie_arqam_add_mce_button');
...
class arqam_counter_widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ){
		...
		$instance['layout'] 	= ! empty( $new_instance['layout'] )   ? $new_instance['layout']   : false;
		$instance['columns'] 	= ! empty( $new_instance['columns'] )  ? $new_instance['columns']  : false;
		$instance['title'] 		= ! empty( $new_instance['title'] )    ? $new_instance['title']    : false;
		$instance['dark'] 		= ! empty( $new_instance['dark'] )     ? $new_instance['dark']     : false;
		$instance['width'] 		= ! empty( $new_instance['width'] )    ? $new_instance['width']    : false;
		$instance['box_only'] = ! empty( $new_instance['box_only'] ) ? $new_instance['box_only'] : false;
